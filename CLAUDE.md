...
- RunPod **Global volumes** (BETA, object storage, mounted at `/workspace`) are **not recommended with this template** (owner ruling): H3 model loading reads over the network (a 5 s AiAngelH3 clip took 12 min 36 s end to end, 20 s of it sampling, vs 192 s cold / 30 s warm on pod-local disk), the only clip ever made on one came out a colour mosaic (model files written there by aria2 were checked by size only; cause unconfirmed), and they now bill $0.09/GB + IOPS. Recommend a Network volume or no volume. The code still tolerates them: they refuse temp files (`mkstemp`) and `chmod`, so `start.sh` syncs nodes with `rsync --inplace` + per-node `__init__.py` heal + symlink fallback, and key saving ignores a refused chmod. `/workspace` reports 0 used of 1 PiB there and also on a pod with no volume, so the dashboard shows "size not reported".
- The node's bundle-version hash (Dockerfile `AIANGEL_NODE=`) covers every file of `nodes/ComfyUI-AiAngel`, so a change to example workflows alone still re-syncs the node onto existing volumes.
