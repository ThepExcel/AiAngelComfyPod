...
- `.github/workflows/build.yml` builds and pushes `ghcr.io/thepexcel/aiangelcomfypod` on push to main as a matrix: `:cuda12.8` (also `:latest`) and `:cuda13.0`, same RunPod ComfyUI release pinned by digest. `latest` moves to 13.0 only after a pod A/B shows it faster; a CUDA 13 image needs a host driver that supports it (RunPod `allowedCudaVersions` is a list; hosts report drivers up to 13.2, so filter with several values). No local docker on SiraPC — CI is the only build.
...
- Pod tests (tooling in `D:/ClaudeMediaGen/scripts/runpod/`) have no budget cap: keep a test pod up until the test plan is done and the owner has seen the results, arm the deadman, report the running cost. Live iterating runs on an RTX PRO 6000 (about 2.5x a 5090 per clip); recipes and timings live in that folder's README.
