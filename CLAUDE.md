...
- `scripts/h3_workflows.py clip(upscale=True, upscale_scale=…)` adds the H3 latent upscaler + a 3-step refine after sampling (the node author's r2v example; refine sigmas unverified on our recipe); it needs `MODELS=...,h3upscaler` on the pod. 2x of a 10 s 576p clip overflows the serverless 7 MB result. That repo stays private: it re-hosts Civitai authors' LoRAs and a merge of third-party checkpoints.
